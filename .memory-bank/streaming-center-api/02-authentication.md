...
Часть запросов к Streaming.Center API требует аутентификации через API ключ. Ключ передается в HTTP заголовке запроса.
...
**Важно:** API ключ предоставляет доступ к данным и операциям, которые разрешены этим ключом. Храните его в секрете и не передавайте третьим лицам.
...
**Критически важно:** Всегда используйте HTTPS при работе с API ключами. На сайте отдельно отмечено, что при запросах с авторизацией HTTPS критически необходим.
